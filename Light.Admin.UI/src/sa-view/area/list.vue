        ...
        <div class="c-panel">
            ...
            <el-form :inline="true" size="mini">
                ...
                <el-form-item label="邮编：">
                    <el-input v-model="p.postalCode" placeholder="模糊查询"></el-input>
                </el-form-item>
                <el-form-item style="min-width: 0px">
...
export default {
    ...
    methods: {
        // 数据刷新
        f5: function () {
            this.sa.put("/Area/listPage", this.p).then((res) => {
        ...
        del: function (data) {
            this.sa.confirm(
                "是否删除，此操作不可撤销",
                function () {
                    this.sa.delete("/Area/delete/" + data.id).then((res) => {
                        console.log(res);
        ...
        exportFile: function () {
            ...
            var titleObj = [
                ...
                { key: "depth", name: "地区深度" },
                { key: "name", name: "地区名称" },
                { key: "postalCode", name: "邮编" },
                { key: "sort", name: "地区排序" },
                { key: "id", name: "id" },
            ];
            this.sa
                .exportFile("/Area/listPage", this.p, titleObj)
                .then((res) => {
                    console.log(res);
                });
        },
