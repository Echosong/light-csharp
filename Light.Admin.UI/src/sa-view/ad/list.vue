            ...
            <el-table :data="dataList" size="mini" ref="multipleTable">
                ...
                <el-table-column label="图片">
                    <template slot-scope="s">
                        <el-image style="width: 100px; height: 50px" v-for="item in (s.row.imageFile)" :key="item.name"  :src="item.url" :preview-src-list="[item.url]"></el-image>
                    </template>
                </el-table-column>
                <el-table-column label="位置" prop="postionEnum"></el-table-column>
    ...
    methods: {
        ...
        del: function (data) {
            this.sa.confirm(
                "是否删除，此操作不可撤销",
                function () {
                    this.sa.delete("/Ad/delete/" + data.id).then((res) => {
                        console.log(res);
        ...
        exportFile: function () {
            ...
            var titleObj = [
                ...
                { key: "title", name: "标题" },
                { key: "image", name: "图片" },
                { key: "postionEnum", name: "位置" },
                { key: "url", name: "链接" },
                { key: "id", name: "id" },
            ];
            this.sa.exportFile("/Ad/listPage", this.p, titleObj).then((res) => {
                console.log(res);
            });
        },
