...
<script>
	...
	export default {
		...
		methods: {
			// 数据刷新
			f5: function() {
				this.sa.put("/Tag/listPage", this.p).then(res=>{
			...
			del: function(data) {
				this.sa.confirm('是否删除，此操作不可撤销', function() {
					this.sa.delete("/Tag/delete/"+ data.id).then(res =>{
						console.log(res)
			...
			exportFile: function(){
				...
				var titleObj = [{key:"typeEnum",name:"类型" },{key:"name",name:"标签名称" },{key:"description",name:"描述" },{key:"sort",name:"排序" },{key:"id",name:"id" }] ;
				this.sa.exportFile("/Tag/listPage", this.p, titleObj).then(res => {
					console.log(res);
				});
			},
