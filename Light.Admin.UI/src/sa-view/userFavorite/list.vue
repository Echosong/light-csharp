        ...
        <div class="c-panel">
            ...
            <el-table :data="dataList" size="mini" ref="multipleTable">
				...
				-->
                   ...
                   <el-table-column  label="相关id"   prop="relateId" ></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			// 数据刷新
			f5: function() {
				this.sa.put("/UserFavorite/listPage", this.p).then(res=>{
			...
			del: function(data) {
				this.sa.confirm('是否删除，此操作不可撤销', function() {
					this.sa.delete("/UserFavorite/delete/"+ data.id).then(res =>{
						console.log(res)
			...
			exportFile: function(){
				...
				var titleObj = [{key:"userId",name:"用户id" },{key:"userName",name:"用户名" },{key:"typeEnum",name:"类型" },{key:"relateId",name:"相关id" },{key:"id",name:"id" }] ;
				this.sa.exportFile("/UserFavorite/listPage", this.p, titleObj).then(res => {
					console.log(res);
				});
			},
