    ...
    <div class="vue-box">
        <div class="c-panel">
            ...
            <el-table :data="dataList" size="mini" ref="multipleTable">
				...
				-->
                <el-table-column label="目标用户" prop="userId"></el-table-column>
...
export default {
    ...
    methods: {
        // 数据刷新
        f5: function () {
            this.sa.put("/Notice/listPage", this.p).then((res) => {
        ...
        del: function (data) {
            this.sa.confirm(
                "是否删除，此操作不可撤销",
                function () {
                    this.sa.delete("/Notice/delete/" + data.id).then((res) => {
                        console.log(res);
        ...
        exportFile: function () {
            ...
            var titleObj = [
                { key: "userId", name: "目标用户0位全部,-1 vip" },
                { key: "title", name: "标题" },
                { key: "description", name: "内容" },
                { key: "stateEnum", name: "状态" },
                { key: "id", name: "id" },
            ];
            this.sa
                .exportFile("/Notice/listPage", this.p, titleObj)
                .then((res) => {
                    console.log(res);
                });
        },
