<template>
 ...
 <el-table-column  label="文件路径"  >
    <template slot-scope="s">
        <el-link  v-for="item in (s.row.urlFile)"  :key="item.name" :href="item.url"  type="primary" >{{item.name}}</el-link>
     </template>
 </el-table-column> 
                   <el-table-column  label="VIP免费"   prop="vipFree" ></el-table-column>
                   <el-table-column  label="首页显示"   prop="index" ></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			// 数据刷新
			f5: function() {
				this.sa.put("/Download/listPage", this.p).then(res=>{
					this.dataList = res.data.content.map((item) => {
					     item.urlFile = JSON.parse(item.url);
                        return item;
                    });;
			...
			del: function(data) {
				this.sa.confirm('是否删除，此操作不可撤销', function() {
					this.sa.delete("/Download/delete/"+ data.id).then(res =>{
						console.log(res)
			...
			exportFile: function(){
				...
				var titleObj = [{key:"title",name:"文件名" },{key:"price",name:"价格" },{key:"url",name:"文件路径" },{key:"vipFree",name:"VIP免费" },{key:"index",name:"首页显示" },{key:"id",name:"id" }] ;
				this.sa.exportFile("/Download/listPage", this.p, titleObj).then(res => {
					console.log(res);
				});
			},
