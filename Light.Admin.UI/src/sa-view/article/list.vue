    ...
    <div class="vue-box">
        <div class="c-panel">
            ...
            <el-table :data="dataList" size="mini" ref="multipleTable">
                ...
                <el-table-column label="分类" prop="typeEnum"></el-table-column>
                <el-table-column label="图片">
                    <template slot-scope="s">
                        <el-image style="width: 60px; height: 60px; margin-left: 10px; " v-for="item in (s.row.imgFile)" :key="item.id" :src="item.url" :preview-src-list="[item.url]" type="primary">{{item.name}}</el-image>
                    </template>
                </el-table-column>
                <el-table-column label="作者" prop="author"></el-table-column>
                <el-table-column label="时间" prop="time"></el-table-column>
                <el-table-column label="简介" :show-overflow-tooltip="true" prop="info"></el-table-column>
...
export default {
    ...
    data() {
        return {
            p: { pageSize: 10, page: 1, title: "", type: "" },
            dataCount: 0,
            dataList: [],
        };
    },
    methods: {
        ...
        f5: function () {
            this.sa.put("/Article/listPage", this.p).then((res) => {
                this.dataList = res.data.content.map((item) => {
                    try {
                        item.imgFile = JSON.parse(item.img);
                    } catch (e) {
                        item.imgFile = [{ url: item.img }];
                    }
                    return item;
                });
        ...
        exportFile: function () {
            ...
            this.sa
                .exportFile("/Article/listPage", this.p, titleObj)
                .then((res) => {
                    console.log(res);
                });
        },
