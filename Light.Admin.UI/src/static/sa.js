...
(function() {
	...
	var cfg_test = {
		api_url: '/api',
		web_url: 'http://localhost:7045/'
	}
	// 服务器测试环境
	var cfg_prod = {
		api_url: 'http://v-admin.tongchenghy.cn',
		web_url:  'http://v-admin.tongchenghy.cn/'
	}
...
(function() {
	...
	sa.ajax = function(url, data, success200, cfg) {
		...
		var defaultCfg = {
			...
			success410: function(res) { // 410 都是未登录，获取唯有权限
				sa.loginState = false;
			...
			success10000: function(res) { // code=500, 代表失败 
				Notification({
					title: "异常",
					message: res.message ? (res.message.length > 100 ? res.message.substr(0,
						100) : res.message) : "接口异常10000",
					type: 'error'
				})
			},
	...
	sa.exportFile = function(url, data, titleObj) {
		return new Promise((suc, fail) => {
			sa.ajax(url, data, function(res) {
				let jsonData = res.data.content;
				//列标题，逗号隔开，每一个逗号就是隔开一个单元格
				let str = "";
				for (let j=0;j<titleObj.length;j++) {
					str += `${[titleObj[j].name] + '\t'},`;		
				}
				str +=  `\n`;
				//第二行读取key
				...
				link.href = uri;
				//对下载的文件命名
				link.download = url.replace("/", "_") + ".csv";
	...
	//获取用户信息
	sa.getUsernameById = function(userId){
		if(!userId) return "";
		var userInfoStr = localStorage.getItem("g-user-info");
		var userList = [];
		if(userInfoStr){
			userList = JSON.parse(userInfoStr);
			var ones = userList.filter(t=>t.id == userId);
			if(ones){
				return ones[0]['name'];
			}else{
				return userId;
